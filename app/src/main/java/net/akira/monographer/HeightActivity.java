...
public class HeightActivity extends FragmentActivity implements OnClickListener{
    ...
    private double p1;  //出發點
    private double p2;  //結束點
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (info == null) {
            new AlertDialog.Builder(HeightActivity.this)
                    .setTitle("系統訊息")
            ...
            try {
                ...
                if (mLocation != null) {
                    p1 = mLocation.getAltitude();
                    p2 = p1;
                    tv_show_gps.setText(
                            "海拔： " + mLocation.getAltitude() + " m"
                    );
                }
            } catch (Exception e) {
                new AlertDialog.Builder(HeightActivity.this)
    ...
    //計算差距，單位是公尺
    private double gps2m(double gp1, double gp2) {
        double radLat1 = gp2;
        double radLat2 = gp1;
    ...
    public void onClick(View v) {
        switch(v.getId()){
            case R.id.myButton1:    //開始記錄
                ...
                mLocationManager.requestLocationUpdates(mLocationPrivider, 1000, 0, mLocationListener);
                p1 = mLocation.getAltitude();
                start_point.setText("起始高度： " + p1);
                break;
            case R.id.myButton2:    //結束記錄
                getLocationPrivider();
                mLocationManager.requestLocationUpdates(mLocationPrivider, 1000, 0, mLocationListener);
                p2 = mLocation.getAltitude();
                finish_point.setText("起始高度： " + p2);
                break;
            case R.id.myButton3:    //我的路徑
                height = gps2m(p1, p2);
                show_distance.setText(
                        "測驗結果: " + disformat(100 * height) +" cm"
                );
                break;
